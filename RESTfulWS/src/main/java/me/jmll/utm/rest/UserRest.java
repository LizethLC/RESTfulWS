...
public class UserRest {
	...
	@RequestMapping(value = "user", method = RequestMethod.GET, produces = { "application/json", "text/json" })
	@ResponseBody
	...
	public Map<String, Object> getUsersJSON() {
		...
		userService.getUsers().forEach(user -> data.add(new Link(ServletUriComponentsBuilder.fromCurrentServletMapping()
	...
	public UserLinkListResource getUsersXML() {
		...
		userService.getUsers().forEach(
				user -> userLinksResource.addUserLink(new Link(ServletUriComponentsBuilder.fromCurrentServletMapping()
						.path("/user/" + user.getUsername()).build().toString(), user.getUsername())));
